...
/// Form page for creating or editing a purchase order.
///
/// Provides a complete order entry form with:
/// - Supplier selector (searchable dropdown loaded from database)
...
/// Otherwise, a blank form is shown for creating a new purchase order.
...
class _PurchaseOrderFormPageState extends State<PurchaseOrderFormPage> {
  ...
  void initState() {
    ...
    if (_isEditMode) {
      ...
      _selectedSupplierName = o.supplierName;
  ...
  Future<void> _submitOrder(String status) async {
    ...
    context.read<OrdersBloc>().add(CreatePurchaseOrder(order: order));
  }
                  ...
                  children: [
                    Expanded(
                      child: OutlinedButton(
                        onPressed:
                            _isSaving ? null : () => _submitOrder('draft'),
        ...
        InkWell(
          ...
          child: InputDecorator(
            decoration: InputDecoration(
              hintText: 'Select supplier (optional)',
            ...
            child: Text(
              _selectedSupplierName ?? 'No supplier selected',
              style: TextStyle(
  ...
  Future<void> _showSupplierPicker(BuildContext context) async {
    ...
    if (selected != null && mounted) {
      final supplier = await dao.getSupplierById(selected);
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (ctx, i) {
                          ...
                          return ListTile(
                            title: Text(p.name),
                            subtitle: Text(
                              '₹${(p.purchasePrice != null ? p.purchasePrice! / 100 : p.sellingPrice / 100).toStringAsFixed(2)}',
                            ),
      ...
      if (product != null && mounted) {
        ...
        setState(() {
          _items[index] = currentItem.copyWith(
            ...
            unitPrice: product.purchasePrice ?? product.sellingPrice,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _summaryRow(
              'Tax',
              '₹${(_taxAmount / 100).toStringAsFixed(2)}',
            ),
            _summaryRow(
              'Discount',
