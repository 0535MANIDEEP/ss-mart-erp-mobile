...
/// Domain entity representing a line item within a [SalesOrder].
///
/// Each item captures the product, quantity ordered, pricing, and tax details
/// for a single product on the order. Denormalized product name preserves
/// historical accuracy even if the product master is later modified.
///
/// ## Delivery Tracking
///
/// [deliveredQuantity] tracks partial deliveries. When the full quantity is
/// delivered, this equals [quantity]. Partial deliveries allow progressive
/// fulfilment of large orders across multiple shipments.
///
...
class SalesOrderItem extends Equatable {
  ...
  /// Foreign key linking back to the parent [SalesOrder].
  ...
  /// Unit selling price in paise at the time the order was placed.
  final int unitPrice;
  ...
  /// Quantity that has been delivered so far. Supports partial deliveries.
  /// Must be <= [quantity].
  ...
  /// Whether partial delivery has been made but not yet complete.
  bool get isPartiallyDelivered =>
      deliveredQuantity > 0 && deliveredQuantity < quantity;

  /// Remaining quantity yet to be delivered.
  double get pendingQuantity => quantity - deliveredQuantity;
  ...
  /// Creates a copy of this sales order item with optional field overrides.
